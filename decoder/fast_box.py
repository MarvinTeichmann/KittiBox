...
def apply_rezoom(hyp, phase, early_feat, raw_output, pred_box,
    ...
    return pred_confidences, pred_confs_deltas, pred_boxes_deltas
...
def _apply_simple_rezoom(hyp, train, scored_feat, early_feat):
    ...
    w_offsets = hyp['rezoom_w_coords']
    h_offsets = hyp['rezoom_h_coords']
    with tf.name_scope("Rezoom_Features"):
        rezoom_features = _rezoom(hyp, pred_box, early_feat,
                                  early_feat_channels, w_offsets, h_offsets)

    rezoom_features = tf.reshape(rezoom_features,
                                 [1, hyp['grid_height'],
                                  hyp['grid_width'], -1])

    new_features = tf.concat(3, [scored_feat, rezoom_features / 100.])
...
def decoder(hyp, logits, train):
    ...
    with tf.variable_scope('fast_box', initializer=initializer):
        ...
        pred_box = tf.reshape(scored_feat[:, :, :, 0:4],
                              (-1, hyp['rnn_len'], 4))
        nc = hyp['num_classes']
        pred_logits = tf.reshape(scored_feat[:, :, :, 4:4+nc],
                                 (-1, nc))
