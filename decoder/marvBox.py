...
def _build_decoder_inner(hyp, decoder_input):
    ...
    with tf.variable_scope('Overfeat', initializer=initializer):
        w = tf.get_variable('ip', shape=[hyp['cnn_channels'],
                                         hyp['lstm_size']])
        outputs.append(tf.matmul(decoder_input, w))
    return outputs


def _build_fc_inner(hyp, decoder_input):
    '''
    build simple overfeat decoder
    '''
    if hyp['rnn_len'] > 1:
    ...
    outer_size = grid_size * hyp['batch_size']
    initializer = tf.random_uniform_initializer(-0.1, 0.1)
    decoder_input = tf.reshape(decoder_input, [hyp['batch_size'],
...
def decoder(hyp, logits, train):
    ...
    with tf.variable_scope('decoder', initializer=initializer):
        ...
        lstm_outputs = _build_decoder_inner(hyp, decoder_input)
